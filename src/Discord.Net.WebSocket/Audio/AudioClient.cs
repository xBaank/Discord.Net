...
public partial class AudioClient : IAudioClient
{
    ...
    public IReadOnlyDictionary<ulong, AudioInStream> GetStreams() => _streams.ToDictionary(pair => pair.Key, pair => pair.Value.Reader);
    ...
    private async Task OnDisconnectingAsync(Exception ex)
    {
        ...
        if (!_isChangingChannel)
            ...
            await Discord.ApiClient.SendVoiceStateUpdateAsync(Guild.Id, ChannelId, false, false).ConfigureAwait(false);
    }
    ...
    {
        if (_streams.TryGetValue(id, out var streamPair))
            ...
            else if (_connection.State == ConnectionState.Connected)
            {
                if (packet.Length == 8)
                {
                    ...
                    while (_keepaliveTimes.TryDequeue(out var pair))
                    {
                        if (pair.Key == value)
                        {
                            var latency = Environment.TickCount - pair.Value;
        ...
        try
        {
            ...
            while (!cancelToken.IsCancellationRequested)
            {
                ...
                if (_heartbeatTimes.Count != 0 &&
                    now - _lastMessageTime > intervalMillis &&
