...
public class RTPWriteStream : AudioOutStream
{
    ...
    public override async Task FlushAsync(CancellationToken cancelToken) => await _next.FlushAsync(cancelToken).ConfigureAwait(false);

    public override async Task ClearAsync(CancellationToken cancelToken) => await _next.ClearAsync(cancelToken).ConfigureAwait(false);
