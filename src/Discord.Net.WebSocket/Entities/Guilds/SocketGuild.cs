...
public class SocketGuild : SocketEntity<ulong>, IGuild, IDisposable
{
    ...
    public IAsyncEnumerable<IReadOnlyCollection<RestAuditLogEntry>> GetAuditLogsAsync(int limit, RequestOptions options = null, ulong? beforeId = null, ulong? userId = null, ActionType? actionType = null)
        => GuildHelper.GetAuditLogsAsync(this, Discord, beforeId, limit, options, userId, actionType);
...
#pragma warning restore IDISP002, IDISP006
    ...
    /// </summary>
    public SocketGuildUser CurrentUser => _members.TryGetValue(Discord.CurrentUser.Id, out var member) ? member : null;
    ...
    internal void Update(ClientState state, ExtendedModel model)
    {
        ...
        var members = new ConcurrentDictionary<ulong, SocketGuildUser>(ConcurrentHashSet.DefaultConcurrencyLevel, (int)(model.Members.Length * 1.05));
        {
            ...
            {
                if (members.TryGetValue(model.Presences[i].User.Id, out var member))
        ...
        {
            for (var i = 0; i < model.GuildScheduledEvents.Length; i++)
            {
    ...
    internal void Update(ClientState state, Model model)
    {
        ...
        if (model.Stickers != null)
        {
            ...
            for (var i = 0; i < model.Stickers.Length; i++)
            {
    ...
    /// <param name="options">The options to be used when sending the request.</param>
    /// <returns>
    ///     A task that represents the asynchronous creation operation. The task result contains a collection of commands that
    ///     was created.
    /// </returns>
    ...
    ///     role.
    /// </returns>
    public Task<RestRole> CreateRoleAsync(string name, GuildPermissions? permissions = default, Color? color = default,
    ...
    {
        if (_roles.TryRemove(id, out var role))
            return role;
        return null;
    }
    ...
    {
        if (_stickers.TryRemove(id, out var sticker))
            return sticker;
        return null;
    }
    ...
    {
        if (_members.TryRemove(id, out var member))
        {
            DownloadedMemberCount--;
    ...
    ///     <remarks>
    ///         The event must have a type of <see cref="GuildScheduledEventType.Stage" /> or
    ///         <see cref="GuildScheduledEventType.Voice" />
    ...
    internal async Task<IAudioClient> ConnectAudioAsync(ulong channelId, bool selfDeaf, bool selfMute, bool external)
    {
        ...
        try
        {
            ...
            {
                GuildId = Id, ChannelId = channelId, SelfDeaf = selfDeaf, SelfMute = selfMute,
            };
        ...
        ulong? beforeId, ulong? userId, ActionType? actionType)
    {
        if (cacheMode == CacheMode.AllowDownload)
            return (await GetAuditLogsAsync(limit, options, beforeId, userId, actionType).FlattenAsync().ConfigureAwait(false)).ToImmutableArray();
